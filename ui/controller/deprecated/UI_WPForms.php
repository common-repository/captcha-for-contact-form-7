...
namespace f12_cf7_captcha\deprecated {
	 ...
	 * Class UIDashboard
	 */
	class UI_WPForms extends UI_Page_Form {
		public function __construct( UI_Manager $UI_manager ) {
			parent::__construct( $UI_manager, 'wp_forms', 'WPForms' );
		}
		...
		protected function on_save( $settings ) {
			...
			foreach ( $default['wpforms'] as $key => $value ) {
				if ( isset( $_POST[ $key ] ) ) {
					if ( is_numeric( $value ) ) {
						$settings['wpforms'][ $key ] = (int) $_POST[ $key ];
					} else {
						$settings['wpforms'][ $key ] = sanitize_text_field( $_POST[ $key ] );
					}
				} else {
					$settings['wpforms'][ $key ] = 0;
				}
			}
		...
		protected function the_content( $slug, $page, $settings ) {
			$settings = $settings['wpforms'];

			?>
			<h2>
				<?php _e( 'WPForms', 'captcha-for-contact-form-7' ); ?>
			</h2>
			...
			<div class="section">
				...
				<div class="option">
					...
					<div class="input">
						...
						/>
						<span>
                        <?php _e( 'Enable the Captcha for every WPForms Form available on this system.', 'captcha-for-contact-form-7' ); ?>
                    </span>
					</div>
				</div>
